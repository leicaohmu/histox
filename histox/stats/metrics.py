...
class ClassifierMetrics:
    ...
    def save_roc(self, outdir, name):
        with hx.util.matplotlib_backend('Agg'):
            ...
            hx.stats.plot.roc(self.fpr, self.tpr, f'AUC = {auroc_str}')
    ...
    def save_prc(self, outdir, name):
        with hx.util.matplotlib_backend('Agg'):
            ...
            hx.stats.plot.prc(self.precision, self.recall, label=f'AP = {ap_str}')
            full_path = join(outdir, f'{name}.png')
            plt.savefig(full_path)
            plt.close()
            if self.neptune_run:
...
def classification_metrics(
    ...
    Args:
        df (pd.DataFrame): Pandas DataFrame containing labels, predictions,
            and optionally uncertainty, as returned by hx.stats.df_from_pred()
...
def eval_from_dataset(*args, **kwargs):
    warnings.warning(
        "`hx.stats.metrics.eval_from_dataset() is deprecated. Please use "
        "`hx.stats.metrics.eval_dataset()` instead.",
        DeprecationWarning)
...
def eval_dataset(
    ...
    if hx.model.is_tensorflow_model(model):
        from histox.model import tensorflow_utils
...
def regression_metrics(
    ...
    if ((sorted(outcome_names) != sorted(_outcomes_by_true))
       or not len(outcome_names)):
        raise ValueError("Improperly formatted dataframe to regression_metrics(); "
                         ...
                         "with histox.stats.name_columns(). "
    ...
    r_squared = hx.stats.plot.scatter(
...
def predict_from_dataset(*args, **kwargs):
    warnings.warning(
        "`hx.stats.metrics.predict_from_dataset() is deprecated. Please use "
        "`hx.stats.metrics.predict_dataset()` instead.",
        DeprecationWarning)
