"""Utility functions for histox.Project."""
...
from histox.util import log, relative_path
...
def _project_config(
    ...
    if not isinstance(dataset_config, str):
        dataset_config_path = hx.util.relative_path('./datasets.json', root)
        dataset_config = hx.util.write_json(dataset_config, dataset_config_path)
        args['dataset_config'] = './datasets.json'

    args['histox_version'] = hx.__version__
    if sources is None:
        args['sources'] = []
...
def _heatmap_worker(
    ...
    """Heatmap worker for :meth:`histox.Project.generate_heatmaps.`
    ...
    """
    hx.setLoggingLevel(args.verbosity)
    heatmap = hx.Heatmap(slide,
                         model=args.model,
...
def _train_worker(
    datasets: Tuple[hx.Dataset, hx.Dataset],
    model_kw: Dict,
    training_kw: Dict,
    results_dict: Dict,
    verbosity: int
    ...
    trainer = hx.model.build_trainer(**model_kw)
...
def _setup_input_labels(
    ...
    val_dts: Optional[hx.Dataset] = None
) -> Tuple[Dict, List[int], Dict]:
    '''
    Args:
        dts (:class:`histox.Dataset`): Training dataset.
        inpt_headers (list(str)): Annotation headers for slide-level input.
        val_dts (:class:`histox.Dataset`, optional): Validation dataset.
    ...
    for inpt in inpt_headers:
        ...
        if is_float:
            ...
            for slide in slides:
                onehot_label = hx.util.to_onehot(
...
def get_validation_settings(**kwargs: Any) -> SimpleNamespace:
    ...
    Args:
        ...
        filters (dict): Filters dictionary to use for validation dataset.
            See :meth:`histox.Dataset.filter` for more information.
            Defaults to None (same as training).
...
def add_source(
    ...
    try:
        datasets_data = hx.util.load_json(path)
    ...
    hx.util.write_json(datasets_data, path)
...
def load_sources(path: str) -> Tuple[Dict, List]:
    ...
    try:
        sources_data = hx.util.load_json(path)
...
def create_ensemble_dataframe(
    ...
    df = hx.util.load_predictions(pred_file)
...
def add_to_ensemble_dataframe(
    ...
    member_df = hx.util.load_predictions(member_file)
    save_format = detect_predictions_format(pred_file)
    df = hx.util.load_predictions(pred_file)
...
def ensemble_train_predictions(ensemble_path: str) -> None:
    ...
    Args:
        ensemble_path (str): Path to directory containing ensemble members,
            as generated by :meth:`histox.Project.train_ensemble()`.
    """
    ...
    params = hx.util.load_json(join(ensemble_path, 'ensemble_params.json'))
...
def detect_predictions_format(path: str):
    ...
    if path.endswith("csv"):
        ...
        return hx.util.path_to_ext(path)
...
def interactive_project_setup(project_folder: str) -> Dict:
    ...
    project['annotations'] = hx.util.path_input(
    ...
    project['dataset_config'] = hx.util.path_input(
    ...
    while not project['sources']:
        ...
        if not len(sources):
            ...
            selection = hx.util.choice_input(
    ...
    project['models_dir'] = hx.util.path_input(
        'Where should the saved models be stored? [./models] ',
        root=project_folder,
        default='./models',
        create_on_invalid=True
    )
    project['eval_dir'] = hx.util.path_input(
    ...
    hx.util.write_json(settings, join(project_folder, 'settings.json'))
...
class _ProjectConfig:
    ...
    def to_dict(cls):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if hx.util.md5(config_dest) != cls.config_md5:
                raise errors.ChecksumError("Remote config URL failed MD5 checksum.")
            config = hx.util.load_json(config_dest)
        config['annotations'] = cls.labels_url
        config['annotations_md5'] = cls.labels_md5
        return config

class BreastER(_ProjectConfig):
    config_url = 'https://raw.githubusercontent.com/leicaohmu/histox/master/datasets/breast_er/breast_er.json'
    config_md5 = '6732f7e2473e2d58bc88a7aca1f0e770'
    labels_url = 'https://raw.githubusercontent.com/leicaohmu/histox/master/datasets/breast_er/breast_labels.csv'
    labels_md5 = 'e25028e87760749973ceea691e6d63d7'

class ThyroidBRS(_ProjectConfig):
    config_url = 'https://raw.githubusercontent.com/leicaohmu/histox/master/datasets/thyroid_brs/thyroid_brs.json'
    config_md5 = 'c4fbe83766db8f637780f7881cb1045e'
    labels_url = 'https://raw.githubusercontent.com/leicaohmu/histox/master/datasets/thyroid_brs/thyroid_labels.csv'
    labels_md5 = 'c04f2569dc3a914241fae0d0b644a327'

class LungAdenoSquam(_ProjectConfig):
    config_url = 'https://raw.githubusercontent.com/leicaohmu/histox/master/datasets/lung_adeno_squam/lung_adeno_squam.json'
    config_md5 = '9239d18b66e054132700c08831560669'
    labels_url = 'https://raw.githubusercontent.com/leicaohmu/histox/master/datasets/lung_adeno_squam/lung_labels.csv'
