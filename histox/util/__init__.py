...
try:
    addLoggingFileHandler("histox.log")
except Exception as e:
...
def make_cache_dir_path(path: str) -> str:
    if 'HOME' in os.environ:
        dest = os.path.join(os.environ['HOME'], '.cache', 'histox', path)
    elif 'USERPROFILE' in os.environ:
        dest = os.path.join(os.environ['USERPROFILE'], '.cache', 'histox', path)
    else:
        dest = os.path.join(tempfile.gettempdir(), '.cache', 'histox', path)
...
def get_preprocess_fn(model_path: str):
    ...
    if is_torch_model_path(model_path):
        from histox.io.torch import preprocess_uint8
        return partial(preprocess_uint8, normalizer=normalizer)
    elif is_tensorflow_model_path(model_path):
        from histox.io.tensorflow import preprocess_uint8
