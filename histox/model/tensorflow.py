'''Tensorflow backend for the histox.model submodule.'''
...
from histox.util import log, NormFit, no_scope
...
class Trainer:
    ...
    def train(
        ...
        Args:
            train_dts (:class:`histox.Dataset`): Training dataset. Will call
                the `.tensorflow()` method to retrieve the tf.data.Dataset
                used for model fitting.
            val_dts (:class:`histox.Dataset`): Validation dataset. Will call
                the `.tensorflow()` method to retrieve the tf.data.Dataset
                used for model fitting.
...
class Features(BaseFeatureExtractor):
    ...
    def __init__(
        ...
        """Creates a features interface from a saved histox model which
    ...
    def from_model(
        ...
        """Creates a features interface from a loaded histox model which
        outputs feature activations at the designated layers.
    ...
    def __call__(
        ...
        **kwargs
        ...
        Expects either a batch of images or a :class:`histox.WSI`.
    ...
    def dump_config(self):
        return {
            'class': 'histox.model.tensorflow.Features',
            'kwargs': {
                'path': self.path,
                'layers': self.layers,
...
class UncertaintyInterface(Features):
    ...
    def dump_config(self):
        return {
            'class': 'histox.model.tensorflow.UncertaintyInterface',
            'kwargs': {
                'path': self.path,
                'layers': self.layers,
                'pooling': self._pooling
